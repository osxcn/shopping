...
<div class="g-doc">
    ...
    <table class="m-table m-table-row n-table g-b3" id="newTable">
        ...
        <tbody>
            <#list buyList as x>
            <#assign total = total + x.payment>
            <tr>
                <td class="img"><a href="/snapshot?id=${x.id}"><img src="${x.image}" alt=""></a></td>
                <td class="title"><h4><a href="/snapshot?id=${x.id}">${x.title}</a></h4></td>
                <td class="time"><span class="v-time">${x.time?number_to_datetime?string("yyyy-MM-dd HH:mm")}</span></td>
                <td class="price"><span class="v-unit">¥</span><span class="value">${x.price}</span></td>
                <td class="num"><span class="v-num">${x.num}</span></td>
                <td class="payment"><span class="v-unit">¥</span><span class="value">${x.payment}</span></td>
            </tr>
            </#list>
        </tbody>
        <tfoot>
